    ...
    function photo($token){
        ...
        if (isset($_GET['platform']) && isset($_GET['id'])) {
            if ($_GET['platform'] === 'unsplash') {
                ...
                $data = ["link" => $info['urls']['full'],"platform"=>"unsplash","create_data" => $info['created_at'],"width"=>$info['width'],"height"=>$info['height'], "likes" => $info['likes'], "downloads" => $info['downloads'], "alt" => $alt];
